...
const Item = ({id, title, image, description, price, category, rating, isFull, addToCard}) => {

  return (
    <div className='bg-white p-4 mb-4 rounded-lg shadow-md hover:shadow-xl flex'>
      <div className='h-52 w-56 mr-8 flex'>
        <img src={image} alt='' className='max-h-52 max-w-52  self-center'/>
      </div>
      <div className='grow flex flex-col justify-around'>
        {isFull ? (
          <h2 className='text-xl pt-4 font-bold mb-2'>{title}</h2>
        ) : (
          <Link to={`/products/${id}`} className='text-xl font-bold mb-2 hover:text-purple-600'>{title}</Link>
        )}
      ...
      <div className='flex flex-col justify-between pt-4'>
          <span className='text-purple-700 text-lg font-bold text-right'>
            $ {price}
          </span>
        <button onClick={() => addToCard({id})}
